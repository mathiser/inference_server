...
from database.models import Base, Task, Model
from .mock_ups.mock_models_and_tasks import MockModelsAndTasks


class TestSQLiteImpl(unittest.TestCase):
    """
    This is a tests of functions in api/img/db_rest_impl.py
    """
    ...
    def test_add_task(self, model=None):
        if not model:
            model = self.repo.model1

        echo_model = self.test_add_model(model)

        with open(self.repo.input_zip, "rb") as r:
            task = self.db.add_task(human_readable_ids=[echo_model.human_readable_id],
                                    zip_file=r)
    ...
    def test_get_tasks(self):
        with open(self.repo.input_zip, "rb") as r:
            task0 = self.test_add_task(self.repo.model1)
            task1 = self.test_add_task(self.repo.model2)

        tasks = self.db.get_tasks()
        self.assertEqual(len(tasks), 2)
        self.assertEqual(tasks[0].to_dict(), task0.to_dict())
        self.assertEqual(tasks[1].to_dict(), task1.to_dict())

    def test_add_model(self, model=None):
        if not model:
            model = self.repo.model1
        ...
        model = self.db.get_model_by_id(echo_model.id)
    ...
    def test_get_model_by_id(self):
        model = self.test_add_model(self.repo.model2)
        echo_model = self.db.get_model_by_id(model.id)
        self.assertEqual(model.to_dict(), echo_model.to_dict())

    def test_get_models(self):
        model1 = self.test_add_model(self.repo.model1)
        model2 = self.test_add_model(self.repo.model2)
        models = self.db.get_models()
        self.assertEqual(len(models), 2)
        self.assertEqual(models[0].to_dict(), model1.to_dict())
        self.assertEqual(models[1].to_dict(), model2.to_dict())

    def test_post_output_by_uid(self):
        task = self.test_add_task(self.repo.model1)
