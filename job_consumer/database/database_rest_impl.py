...
class DBRestImpl(DBInterface):
    ...
    def get_task_by_uid(self, uid: str) -> Task:
        res = requests.get(self.api_url + urljoin(os.environ["PRIVATE_TASKS_BY_UID"], uid))
        return Task(**json.loads(res.content))

    def get_input_zip_by_id(self, id: int) -> BinaryIO:
        res = requests.get(self.api_url + urljoin(os.environ["PRIVATE_INPUT_ZIPS_BY_ID"], str(id)), stream=True)
        if not res.ok:
    ...
    def get_model_by_id(self, id: int) -> Model:
        res = requests.get(self.api_url + urljoin(os.environ["PRIVATE_MODELS_BY_ID"], str(id)))
        return Model(**json.loads(res.content))

    def get_model_by_human_readable_id(self, human_readable_id: str) -> Model:
        res = requests.get(self.api_url + urljoin(os.environ["PRIVATE_MODELS_BY_HUMAN_READABLE_ID"], str(human_readable_id)))
        return Model(**json.loads(res.content))

    def get_model_zip_by_id(self, id: int) -> tempfile.TemporaryFile:
        res = requests.get(self.api_url + urljoin(os.environ["PRIVATE_MODEL_ZIPS_BY_ID"], str(id)), stream=True)
        if not res.ok:
            logging.error(f"RES not ok: {res}")
            raise Exception(res.content)
        return self.res_to_tmp_file(res)
    ...
    def post_output_by_uid(self, uid: str, zip_file: BinaryIO) -> Model:
        res = requests.post(self.api_url + urljoin(os.environ['PRIVATE_OUTPUT_ZIPS_BY_UID'], uid),
                            files={"zip_file": zip_file})

        return Model(**res.json())

    def set_task_status_by_uid(self, uid: str, status: int):
        return requests.put(url=urljoin(self.api_url, os.environ["PRIVATE_TASKS"]),
                            params={"uid": uid,
                                    "status": status})
